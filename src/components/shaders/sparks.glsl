...
vec2 iResolution = normalize(uResolution);

const float CIRCLE_SIZE = 1.0;
...
vec3 trace(vec3 rpos, vec3 rdir, vec2 fragCoord) {
	...
	return final + vec3(rand(vec2(fragCoord.x * fragCoord.y, uTime))) * 0.002;
}
...
vec3 camera(vec2 px) {
	...
	return trace(vec3(-40.0, 20.0, -150.0), rdir, px);
}

void main() {
	gl_FragColor = vec4(pow(camera(uv.xy), vec3(0.4545)), 1.0);
}
